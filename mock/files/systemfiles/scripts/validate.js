...
(function($) {
    ...
    $.fn.siValidation = function(options) {
        ...
        var init = function() {
            ...
            alphaNum2 : message.invalidAlphaNum2,
            alphaNumSymbol : message.invalidAlphaNumSymbol,
            ...
            var messagetop = messageArea.offset().top;
            var msgArea = document.getElementsByClassName("validationMessage");
            var msgAreaHeight = $(msgArea).height();
            if (msgAreaHeight > 34 && top > messagetop){
                $(".triangle").css("translate", "0px 18px")
            } else if (msgAreaHeight > 34 && top < messagetop){
                $(".validationMessage").css("translate", "0px -18px")
            }
        ...
        var validator = {
            method : {
                ...
                alphaNum2 : function(element) {
                    var value = element.val();
                    return !this.required(element) || /^[-A-Za-z0-9_\\.\\+]*$/.test(value);
                },
                alphaNumSymbol : function(element) {
                    var value = element.val();
                    return !this.required(element) || /^(?=.*\S)[ -~]+$/.test(value);
                },
